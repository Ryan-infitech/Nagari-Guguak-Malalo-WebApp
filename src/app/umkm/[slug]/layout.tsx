...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    const umkm = await umkmService.getUMKMBySlug(resolvedParams.slug);
...
export async function generateStaticParams() {
  try {
    ...
    return umkmsResponse.data.map((umkm: any) => ({
      slug: umkm.slug,
    }));
  } catch (error) {
...
export default function UMKMDetailLayout({ children }: { children: React.ReactNode }) {
  return children;
}
